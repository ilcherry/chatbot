...
const EmojiPicker: React.FC<EmojiPickerProps> = ({
  ...
  useEffect(() => {
    const updatePosition = () => {
      ...
      if (distanceFromButton > pickerWidth * 0.5) {
        ...
        if (
          centerAlignedLeft >= padding &&
          centerAlignedLeft + pickerWidth <= viewportWidth - padding
        ) {
  ...
  const currentCategory =
    EMOJI_CATEGORIES[activeCategory] || EMOJI_CATEGORIES[1];
  ...
  // 使用 Portal 渲染到 iframe 的 body
  // 虽然组件在 iframe 中渲染，但 JS 代码从父页面加载，所以需要显式获取 iframe 的 document
  const iframe = document.getElementById(
    "chatbot-widget-iframe"
  ) as HTMLIFrameElement;
  const targetBody = iframe?.contentDocument?.body || document.body;

  return createPortal(pickerContent, targetBody);
};
